...
class ReportCommandBuilder
{
	...
	SUBSTRING(rs.title,1,500) as title, rs.status_id as status_id,
		ri.reference_number, ri.start_date,ri.duration_notes ,ri.end_date, ri.checkin_date, ri.date_time_duration ,ri.checkout_date, ri.previous_end_date, ri.credit_count, TIMESTAMPDIFF(SECOND, ri.start_date, ri.end_date) as duration,
							...
							(SELECT GROUP_CONCAT(CONCAT(participant_users.fname, " ", participant_users.lname) SEPARATOR ",")
                            ...
                            (SELECT GROUP_CONCAT(CONCAT(participant_users.fname, " ", participant_users.lname) SEPARATOR ",")
								FROM reservation_users participants INNER JOIN users participant_users ON participant_users.user_id = participants.user_id WHERE participants.reservation_instance_id = ri.reservation_instance_id AND participants.reservation_user_level = 3) as invitee_list,
							(SELECT GROUP_CONCAT(CONCAT(cav.custom_attribute_id,\'=\', cav.attribute_value) SEPARATOR ",")
    ...
    const PROJECT_FRAGMENT = 'AND rs.project like  "%"@project"%"';

    const PROJECT_CLIENT= 'AND rs.client_name like "%"@client_name"%"';
    
    const BILLABLE = 'AND rs.billable = @billable';
    ...
    public function WithBillable($billable)
    {
	...
	private function GetWhereAnd()
	{
        ...
        if (!empty($this->billable))
        {
            ...
            $and->Append(self::BILLABLE);
            $this->AddParameter(new Parameter(ParameterNames::BILL, $bill));
        }
