...
//             $obj->updateReservationIntance($encoded,$this->GetReferenceNumber());
//         }
        ...
        if ($reservationSeries->RequiresNewSeries()) {
            ...
            if($_POST['client_on_sites'] == '') {
                ...
                $_POST['client_on_sites'] = 'Yes';
            }
            
            if($_POST['sample_on_site'] == '') {
                $_POST['sample_on_site'] = 'No';
            } else {
                $_POST['sample_on_site'] = 'Yes';
            }
            ...
            $actualStart = $_POST['actualStartDate'].' '.$_POST['as'];
            $actualEnd = $_POST['actualEndDate']. ' '.$_POST['ae'];
...
class InstanceUpdatedEventCommand extends EventCommand
{
    ...
    public function Execute(Database $database)
    {
        ...
        //print_r($encoded = json_encode($arr3));exit();
        ...
        //print_r($this->instance->RemovedParticipants());exit();
        ...
        foreach ($this->instance->RemovedInvitees() as $inviteeId) {
            ...
            $obj->deleteInvitees($instanceId,$inviteeId);
            
            $removetReservationUser = new RemoveReservationUserCommand($instanceId, $inviteeId);

            $database->Execute($removetReservationUser);
        }
