function Reservation(opts) {
    ...
    var urls3 = options.resourcesUrlhr3
    ...
    ajaxGet(urls3, null, function (data) {
        $.each(data, function (i, unavailableResourceId) {
        	//alert(unavailableResourceId);
            $('#group_user_list_human_engineer option[resource-id-human-engineer="'+unavailableResourceId+'"]').css('background-color', '#808080');
           // $('#group_user_list_human_engineer option[resource-id-human-engineer="'+unavailableResourceId+'"]').text('New');
    ...
    var urls = options.resourcesUrlhr
        .replace('[sd]', elements.beginDate.val())
        .replace('[ed]', elements.endDate.val())
        .replace('[rn]', elements.referenceNumber.val())
        .replace('[st]', elements.beginTime.val())
    ...
    ajaxGet(urls, null, function (data) {
        $.each(data, function (i, unavailableResourceId) {
            ...
            //$('#group_user_list_human_engineer option[resource-id-human-engineer="'+unavailableResourceId+'"]').attr('disabled', true);
           // $('#group_user_list_human_engineer').selectpicker('refresh');
        });
        ...
        $('#group_user_list_human_engineer option[resource-id-human-engineer="'+$.urlParam('rid')+'"]').prop('disabled', true);
        $('#group_user_list_human_engineer').selectpicker('refresh');
    });
...
////        ajaxGet(urls1, null, function (data) {
////            $.each(data, function (i, unavailableResourceId) {
////            	names.push(unavailableResourceId);
////            	
////                $('#group_user_list_human_engineer option[resource-id-human-engineer="'+unavailableResourceId+'"]').prop('disabled', true);
////                $('#group_user_list_human_engineer').selectpicker('refresh');
////                $(".physicalR_"+unavailableResourceId).remove();
////            });
////            //alert(names);
////            var totalHidden = $('#reservation-resources input[type=hidden]').length; // get number of inputs
////            for(var i=0; i<totalHidden; i++) {
////                //alert($("#reservation-resources input[type=hidden]").eq(i).val());
////                $('#primaryResourceId').val($("#reservation-resources input[type=hidden]").eq(i).val());
////            }
////            
////            //$('#primaryResourceId').val("90");
////        });
//        
//        //alert($(':hidden.resourceId').val());
//       
//       
//        
//        //AddResources();
//        //$("#btnAddResources").click();
//        
//        
...
//    });
    ...
    var AddResources = function () {
        ...
        if (checkboxes.length >= 1) {
            ...
            $.each(checkboxes, function (i, checkbox) {
                ...
                displayDiv.append('<div class="resourceName physicalR_' + checkedResourceId + '" id="' + checkedResourceId + '" style="background-color:' + color + '; color:' + textColor + ';">' + '<span class="resourceDetails" data-resourceId="' + checkedResourceId + '">' + checkedResourceName + '</span> ' + '<input class="resourceId rid" type="hidden" name="additionalResources[]" value="' + checkedResourceId + '"/>' + (requiresApproval ? ' <i class="fa fa-lock" data-tooltip="approval"></i> ' : '') + (requiresCheckin ? ' <i class="fa fa-sign-in" data-tooltip="checkin"></i> ' : '') + (!_.isEmpty(autoReleaseMinutes) ? ' <i class="fa fa-clock-o" data-tooltip="autorelease" data-autorelease="' + autoReleaseMinutes + '"></i> ' : '') + '</div>');
            });
        } else {
        	 primaryResourceContainer.find('.resourceName').remove();
             displayDiv = primaryResourceContainer;
    ...
    var InitializeAdditionalResources = function () {
     ...
     ajaxGet(url, null, function (data) {
         ...
         $.each(data, function (i, unavailableResourceId) {
        	 ...
        	 $.ajax({
                 url: 'checkResource.php',
                 type: 'POST',
                 dataType: "json",
                 async: false,
                 data: { unavailableResourceId:unavailableResourceId} ,
                 cache: false,
        	 ...
        	 var checkbox = dialog.find('[resource-id="' + ajaxResult + '"]');
    ...
    var WireUpSaveDialog = function () {
        $('.save').click(function () {
        	...
        	var DelReason = $('#deleteReasonRecurring').val();
        	if(DelReason == "") {
        		//alert('Reason field is required');
        		$("#deleteReasonRecurring").addClass("error");
        		$('.error_message').show();
        		jqXHR.abort();
        	} else {
        		elements.reservationForm.submit();
    ...
    $('#canceling').click(function () {
    	...
    	$("#deleteReasonRecurring").val("Reason To Delete");
        
    });
    
    $("#deleteReasonRecurring").focus(function() {

    	$("#deleteReasonRecurring").val("");
...
//    }
    ...
    $('#project_lead').on('change', function() {
        ...
        $('#project_lead option[project_leader="'+userId+'"]').prop('disabled', true);
        $('#project_lead').selectpicker('refresh');
       // if ($.inArray(userId, participation.addedUsers) >= 0) {
            //return;
        //}
        ...
        function success(response) {
            ...
            if (response !== false) {
                var item = '<div class="user">' + '<a href="#" class="remove" onclick="remove_user('+userId+ ');return false;"><span class="fa fa-remove"></span></a> <a href="#"  class="bindableUser" data-userid="' + userId + '">' + name + '</a><input type="hidden" class="id" name="participantList[]" value="' + userId + '" />' + '</div>';
    ...
    $('#group_user_list').on('change', function() {
        ...
        function success(response) {
            ...
            if (response !== false) {
                ...
                displayDiv2.append('<div class="resourceName" id="' + response["resource_id"] + '" style="background-color:; color:;"><span class="resourceDetails" data-resourceid="' + response["resource_id"] + '" resource-details-bound="1">\n' +
                    '\n' + response["name"] + '</span> <input class="resourceId rid" type="hidden" name="additionalResources[]" value="' + response["resource_id"] + '"></div>');
                var item = '<div class="user">' + '<a href="#" class="remove" onclick="remove_user(' + response["resource_id"] + ');return false;"><span class="fa fa-remove"></span></a> <a href="#" class="bindableUser" data-userid="' + userId + '">' + name + '</a><input type="hidden" class="id" name="invitationList[]" value="' + userId + '" />' + '</div>';
                elements.inviteeList.append(item);
                $('.bindableUser').bindUserDetails();
                participation.addedUsers.push(userId);
    ...
    $('#physical_list').on('change', function() {
        ...
        $("#physical_list").val($("#physical_list option:first").val());
        /*if ($.inArray(userId, participation.addedUsers) >= 0) {
            return;
        }
    ...
    $('#group_user_list_human_engineer').on('change', function() {
    	...
    	var primaryCheck = $('#primaryResourceId').val();
    	var str = $(this).find(":selected").val();
        var res = str.split("=");
        var name = res[0];
        var userId = res[1];
    ...
    ajaxGet(urls3, null, function (data) {
    	var quotations = [];
        $.each(data, function (i, unavailableResourceId) {
        quotations.push(unavailableResourceId);
          if(resourceId == unavailableResourceId) {
        	  var r = confirm("You are about to double book a resource(s). Do you wish to proceed?");
        	  if (r == true) {
        		  ...
        		  $.ajax({
        		   ...
        		   $('#group_user_list_human_engineer option[resource-id-human-engineer="'+res[2]+'"]').prop('disabled', false);
                   $('#group_user_list_human_engineer').selectpicker('refresh');
        	  }
        ...
        if(jQuery.inArray(resourceId, quotations) != -1) {
        ...
        function success(response) {
            if (response !== false) {
                var primaryResourceContainer12 = $('#additionalResources');
                displayDiv22 = primaryResourceContainer12;

                displayDiv22.append('<div class="resourceName" id="' + response["resource_id"] + '" style="background-color:; color:;"><span class="resourceDetails" data-resourceid="' + response["resource_id"] + '" resource-details-bound="1">\n' +
                    '\n' + response["name"] + '</span> <input class="resourceId rid" type="hidden" name="additionalResources[]" value="' + response["resource_id"] + '"></div>');
                var item2 = '<div class="user">' + '<a href="#" class="remove" onclick="remove_user(' + response["resource_id"] + ');return false;"><span class="fa fa-remove"></span></a> <a href="#" class="bindableUser" data-userid="'+userId+'">'+name+'</a><input type="hidden" class="id" name="invitationList[]" value="'+userId+'" />'+'</div>';
                elements.inviteeList.append(item2);
                $('.bindableUser').bindUserDetails();
                participation.addedUsers.push(userId);
                updateInviteeCount();
            } else {
    ...
    participation.addInvitee = function (name, userId) {
        ...
        function success(response) {
            ...
            if ($('input[name=reservationAction]').val() == 'create') {
            	$('#group_user_list_human_engineer option[resource-id-human-engineer="'+response["resource_id"]+'"]').prop('disabled', true);
            	$('#group_user_list_human_engineer').selectpicker('refresh');
                if (response !== false) {
...
//        }
        ...
        $.ajax({
            url: 'check_project_lead.php',
            type: 'POST',
            dataType: "json",
            data: jQuery.param({ userId: userId}) ,
            cache: false,
            success: successeding,
            error: function () {
                alert("error");
            }
        });

        function successeding(responsed)
        {
        	$('#project_lead option[project_leader="'+responsed["user_id"]+'"]').prop('disabled', true);
        	$('#project_lead').selectpicker('refresh');
        	var item = '<div class="user">' + '<a href="#" class="remove"  onclick="remove_user(' + responsed["user_id"] + ');return false;"><span class="fa fa-remove"></span></a> <a href="#" class="bindableUser" data-userid="' + userId + '">' + name + '</a><input type="hidden" class="id" name="participantList[]" value="' + userId + '" />' + '</div>';
...
//{
//	$("#project_lead").val($("#project_lead option:first").val());
//	$("#group_user_list_human_engineer").val($("#group_user_list_human_engineer option:first").val());
//	
//    //$('#'+id).remove();
...
//    $('#group_user_list_human_engineer').selectpicker('refresh');
//    
//    var reservationAction = $('input[name=reservationAction]').val();
//    //if(reservationAction == 'update') {
//    	$(".btnConfirmAddResources").click();
//   // }
//    
//    var reservationAction = $('input[name=reservationAction]').val();
...
function remove_user(id)
{
    ...
    $('#project_lead option[project_leader="'+id+'"]').prop('disabled', false);
    $('#project_lead').selectpicker('refresh');
