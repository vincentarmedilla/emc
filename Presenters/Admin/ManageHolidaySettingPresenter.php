...
class ManageResourcesPresenter extends ActionPresenter
{
    ...
    public function SearchReservations()
    {
        ...
        $filter = HolidayFilter::GetFilter(
            $range->GetBegin(),
            $range->GetEnd(),
            $_POST['beginDateFilter'],
            $_POST['endDateFilter'],
            $_POST['filterScheduleId']); */
        
        //$list = $this->scheduleRepository->GetList(0, 100, null, null, $filter);
        
        $this->page->ShowReservations($_POST['beginDateFilter'], $_POST['endDateFilter'], $_POST['filterScheduleId']);
    }
...
class HolidayFilter
{
     ...
     */
    public static function GetFilter($beginDateFilter, $endDateFilter, $filterScheduleId)
    {
        ...
        if (!empty($beginDateFilter))
        {
            $database = New Connection();
            $db = $database->openConnection();
            
            $prid = $db->prepare("SELECT * FROM holiday_setting
                                         WHERE(begin >= '$beginDateFilter' AND
                                               end <= '$endDateFilter')
                                               OR schedule LIKE '$filterScheduleId'");
            $prid->execute();
            echo $prid;
            $filter = $prid->fetchAll();
