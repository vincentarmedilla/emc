...
class ReservationsSearchFilter
{
    ...
    public static function GetFilter($startDate, $endDate, $userId, $resourceIds, $scheduleIds, $title, $description, $referenceNumber, $projectNo, $clientName, $requestNumber, 
        $physicalResource, $humanResource, $clientOnSite, $sampleOnSite, $projectLead,$billable)
    {
       if(empty($scheduleIds)) {
        ...
        if(!empty($physicalResource) && empty($humanResource))
        {
            ...
            $filter->_And(new SqlFilterEquals(new SqlFilterColumn(TableNames::RESOURCES, ColumnNames::USERID), $physicalResource));
        }
       
        if(!empty($humanResource) && empty($physicalResource))
        {
            $f = new SqlFilterFreeForm(new SqlFilterColumn(TableNames::RESOURCES, ColumnNames::USERID));
            $filter->_And($f);
        }   
        
        if(!empty($clientOnSite))
        {
            if($clientOnSite == "Yes") {
                $filter->_And(new SqlFilterEquals(new SqlFilterColumn(TableNames::RESERVATION_SERIES_ALIAS, ColumnNames::CLIENT_ON_SITE), $clientOnSite));
            }
        }
        
        if(!empty($sampleOnSite))
        {
            if($sampleOnSite == "Yes") {
                $filter->_And(new SqlFilterEquals(new SqlFilterColumn(TableNames::RESERVATION_SERIES_ALIAS, ColumnNames::SAMPLE_ON_SITE), $sampleOnSite));
            }
        }
        
        if($billable == '1') 
        {
            $filter->_And(new SqlFilterEquals(new SqlFilterColumn(TableNames::RESERVATION_SERIES_ALIAS, ColumnNames::BILLABLE), $billable));
        }
        ...
        if (!empty($projectLead))
        {
            ...
            $prid = $db->prepare("SELECT ru.reservation_instance_id, ru.reservation_user_level, u.user_id from  reservation_users ru
                                inner join users u on ru.user_id = u.user_id
                                where u.user_id = '$projectLead' AND ru.reservation_user_level = 2");
            
            $prid->execute();
            $resultplid = $prid->fetchAll();
            
            foreach ($resultplid as $rplid) {
                $rplid = $rplid['reservation_instance_id'];
            }
            $filter->_And(new SqlFilterEquals(new SqlFilterColumn(TableNames::RESERVATION_USERS, ColumnNames::PROJLEADID), $rplid));
        }   
