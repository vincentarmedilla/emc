...
class ReservationMovePresenter
{
	...
	public function PageLoad()
	{
        ...
        //get layoutid
        $getLayout = $obj->getLayout($user->Timezone);
        $getBlock = $obj->getTimeBlock($getLayout['layout_id'],$this->page->GetStartDate());
        $time = new DateTime($this->page->GetStartDate());
        $date = $time->format('Y-m-d');
        $endTime = $getBlock['end_time'];
        if($endTime == '00:00:00') {
            $et = '24:00:00';
        } else {
            $et = $endTime;
        ...
        //convert to utc
        $end = $date.' '.$et;
        $given = new DateTime($end, new DateTimeZone($this->userSession->Timezone));
        $given->setTimezone(new DateTimeZone("UTC"));
        $endDate = $given->format("Y-m-d H:i:s");
        
        
        $existing_series = $obj->getReservationInstance($referenceNumber);
        $get_data_series = $obj->getSeriesData($existing_series['series_id']);
       // $obj->updateReservationInstances($existing_series['series_id'],$endDate);

		$this->handler->Handle($existingSeries, $this->page);
        
        $new_series = $obj->getReservationInstance($referenceNumber);
        $obj->updateReservationInstances($new_series['series_id'],$endDate);
        $obj->updateSeries($new_series['series_id'],$get_data_series['client_name'],$get_data_series['project'],$get_data_series['client_on_site'],$get_data_series['sample_on_site'],$get_data_series['request_no'],$get_data_series['status'],$get_data_series['billable']);
