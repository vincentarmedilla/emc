		...
		<div class="participationText">
			...
			{if $CanViewAdmin}
				...
				<select id="project_lead2" class="form-control selectpicker" data-live-search="true">
					...
					{foreach from=$get_users key=myId item=i name=foo}
						<option project_leader="{$i.user_id}" value="{$i.fname} {$i.lname}({$i.email})={$i.user_id}">
					...
					{foreach from=$ResourceGroupsAsJson|json_decode item=result }
						{foreach from=$result->children item=row}
							{if {$row->email} neq ''}
									...
									<option name="physical_resource" resource-id-human-engineer="{$row->resource_id}" value="{$row->firstname} {$row->lastname}({$row->email})={$row->userid}={$row->resource_id}">
