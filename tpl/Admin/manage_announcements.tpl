...
<div id="page-manage-announcements" class="admin-page">
	...
	<table class="table" id="announcementList">
		...
		<tbody>
		{foreach from=$announce item=i}
			
			<tr class="{$rowCss}" data-announcement-id="{$i.announcementid }">
				<td class="announcementText">{$i.announcement_text}</td>
				<td class="announcementPriority">{$i.priority}</td>
				<td class="announcementStart">{$i.start_date|date_format:"%B %e, %Y"}</td>
				<td class="announcementEnd">{$i.end_date|date_format:"%B %e, %Y"}</td>
				
				{*<td class="announcementDisplayPage"></td>*}
