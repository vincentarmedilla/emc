...
class ResourceAvailabilityRule implements IReservationValidationRule
{
    ...
    public function Validate($reservationSeries, $retryParameters = null)
    {
        ...
        foreach ($conflicts as $objr) {
            
            if(!isset($objr->Client_name)) {
                $bl[] = 1;
            }
            
            $r = $obj->getResourceData($objr->ResourceId);
        ...
        $mrg = array_merge($hr,$pr);
        
        $t = array_merge($mrg,$bl);
        
        $ck = implode('', $t);
        
        
        $search = '1';
        if(preg_match("/{$search}/i", $ck)) {
