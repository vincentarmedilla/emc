...
class Queries
{
    ...
    const ADD_RESERVATION_SERIES =
        'INSERT INTO
        reservation_series (date_created, title, description, allow_participation, allow_anon_participation, repeat_type, repeat_options, type_id, status_id, owner_id, terms_date_accepted, last_action_by, client_name, project, client_on_site, sample_on_site,request_no,status,billable,total_ducation)
		VALUES (@dateCreated, @title, @description, @allow_participation, false, @repeatType, @repeatOptions, @typeid, @statusid, @userid, @terms_date_accepted, @last_action_by, @client, @project, @cos, @sos, @rn, @status, @billable, @totalDuration)';
   ...
   const DELETE_USER_GROUP = 'DELETE FROM user_groups WHERE user_id = @userid AND group_id = @groupid';
    ...
    const REMOVE_RESERVATION_USER =
        'DELETE FROM reservation_users WHERE reservation_instance_id = @reservationid AND user_id = @userid AND reservation_user_level = 0';
