...
where a.reference_number = '$rf' ");
        ...
        $obj->getAdminResUsers($user->AdminGroups);
       
        $uids = array();
        
        foreach($obj->getUsers($user->Groups) as $users)
        {
            $uids[] =  $users['user_id'];
        }
        ...
        foreach($obj->getUserWithPermission($uids,$_GET['rid'],$obj->getMainResource($_GET['rn']),$obj->getMainResource($_GET['srn'])) as $ad)
        {
            $admin_id[] = $ad['user_id'];
        }
        
        //print_r($admin_id);
        //print_r($obj->getUserGroupsAll($user->Groups));
       
        $mrge = array_merge($obj->getUserGroupsAll($user->Groups),$obj->getUserPermissions($admin_id));
        
        $data = array_reverse($mrge);
        
        $result_admin = array_reverse( // Reverse array to the initial order.
        ...
        $acts = explode(" ",$result1['actualstart']);
        
        $acte = explode(" ",$result1['actualend']);
        
        $checkUnt = $obj->getReservationUntil($reservation_instances['series_id']);
        
        if($checkUnt['repeat_options'] == "") {
            $end = $reservation_instances['end_date'];
        } else {
            $untilEnd = explode("|",$checkUnt['repeat_options']);
            $untilEnd1 = explode("=",$untilEnd[1]);
           // $end = $untilEnd1[1];
            $end = $reservation_instances['end_date'];;
        }
        ...
        $sdate = new DateTime($reservation_instances['start_date']);
        $edate = new DateTime($end);
        
        $DateMiddle = $this->getDatesFromRange($sdate->format('Y-m-d'), $edate->format('Y-m-d'));
        
        $dura = json_decode($reservation_instances['date_time_duration']);
        ...
        $reservationUsers = $obj->getUsersReservation($_GET['rn']);
        
        $rUsers = array_map('current', $reservationUsers);
       
        if (in_array($user->UserId, $rUsers)) {
            $allowed = '1';
        } else {
            $allowed = '0';
        }
        
        $this->Set('allowedCheckin', $allowed);
        $this->Set('durationNotes',$reservation_instances['duration_notes']);
        $this->Set('currentDate',$currentDate);
        $this->Set('startDate',$sdate->format('Y-m-d'));
        $this->Set('total_hours',$this->secondsToTime($total_s));
        $this->Set('durations',json_decode($reservation_instances['date_time_duration']));
        $this->Set('date_middle',$DateMiddle);
        ...
        $this->Set('title',$result1['title']);
        $this->Set('sos',$result1['sample_on_site']);
        $this->Set('billable',$result1['billable']);
        $this->Set('physical',$resource_id_physical);
        $this->Set('resource',$result_qrid);
        $this->Set('resource_id',$resource_id);
        $this->Set('total_duration',$result1['total_ducation']);
        //$this->Set('group_users',$obj->getUserGroups($user->Groups));
        $this->Set('admin_users',/*$obj->getUserPermissions(array_unique($admin_id))*/$result_admin);
        $this->Set('group_users',$obj->getAllUsersInGroup($user->Groups));
        $this->Set('get_users',$obj->getAllUsers());
        $this->Set('actualstartDate',$acts[0]);
        $this->Set('actualendDate',$acte[0]);
        $this->Set('actualstart',$acts[1]);
        $this->Set('actualend',$acte[1]);
        $this->Set('notes',$result1['notes']);
