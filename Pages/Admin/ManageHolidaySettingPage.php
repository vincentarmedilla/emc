...
class ManageHolidayPage extends ActionPage implements IManageResourcesPage
{
    /**
    ...
    public function ProcessPageLoad()
    {
        ...
        $b =  $_GET['beginDateFilter'];
        $e =  $_GET['endDateFilter'];
        $s =  $_GET['scheduleId'];
        
        $userGroup = array_unique($one);
        
        $g = $obj->getHolidaySetting();
        
        if($b != "" || $e != "" || $s != "") {
            $f = $obj->getFilterHolidaySetting($b, $e, $s);
        } else {
            $f = $obj->getHolidaySetting();
        }
            
        $schedule_result = $obj->fetchScheduleByGroup($userGroup);
        $this->Set('user',$user->Groups);
        $this->Set('schedules',$schedule_result);
        
/*      $this->Set('schedule',$f['schedule']);
        $this->Set('begin',$f['begin']);
        $this->Set('end',$f['end']);
        $this->Set('reason',$f['reason']);
        $this->Set('createdby',$f['createdby']);
 */        
        $this->Set('fname',$user->FirstName);
       ...
       /*  echo '<pre>';
        ...
        $this->Display('Admin/HolidaySetting/manage_holiday_setting.tpl');
    }
